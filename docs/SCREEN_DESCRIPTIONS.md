...
- 분류 결과가 예시 데이터다. S1과 이어지면 props로 받는다 (`TODO(R1)`)
- `다시 시도`는 상태만 `분류 중`으로 바꾼다. 실제 재호출은 분류 API를 붙일 때 (`TODO(R1)`)
- `저장할 폴더 고르기`는 S3가 없어서 아직 아무 일도 하지 않는다
- 화면을 보려면 주소 끝에 `#s2`를 붙인다(`localhost:5173/#s2`). S1에서 넘어오게 이으면 없앤다
...
- **미분류 파일을 어디에 저장할지** — 미분류도 저장한다는 건 정해졌다([0007](adr/0007-supported-formats.md), 이 문서 맨 위 원칙). [0013](adr/0013-partial-classify-failure.md)에 있던 "저장 대상에서 뺀다"는 2026-09-18에 뺐다. 남은 건 폴더 이름이다. `미분류` 폴더를 만들지, 최상위에 그냥 둘지는 S3에서 정한다

A·B·C는 [0013](adr/0013-partial-classify-failure.md)·[0011](adr/0011-partial-save-failure.md)·[0012](adr/0012-unsupported-browser.md)로 정해졌다.
