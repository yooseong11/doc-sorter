...
| 상태 | 캡처 | 보이는 것 |
|---|---|---|
| 빈 화면 | ![빈 화면](screens/s1-01-empty.png) | `파일을 선택하면 여기에 목록이 나옵니다.` 파일 선택 버튼과 포트폴리오용 `샘플로 바로 해보기` 버튼을 둔다. 드롭존은 R3 |
...
- `샘플로 바로 해보기`는 정적 샘플 5개를 fetch한 뒤 `File`로 바꿔 일반 파일 선택과 같은 읽기 경로에 넣는다. 포트폴리오 전용 진입점이다.
